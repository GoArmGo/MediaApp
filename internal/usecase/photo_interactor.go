...
type photoUseCase struct {
	photoStorage PhotoStorage // Зависимость от порта PhotoStorage
	photoFetcher PhotoFetcher // Зависимость от порта PhotoFetcher
	fileStorage  FileStorage
}

// NewPhotoUseCase создает новый экземпляр PhotoUseCase.
// Он принимает реализации портов PhotoStorage и PhotoFetcher.
func NewPhotoUseCase(storage PhotoStorage, fetcher PhotoFetcher, fileStorage FileStorage) PhotoUseCase {
	return &photoUseCase{
...
func (uc *photoUseCase) SearchAndSavePhotos(ctx context.Context, query string, page, perPage int) ([]domain.Photo, error) {
	...
	for _, photo := range externalPhotos {
		...
		photo.S3URL = s3URL // Сохраняем полученный S3 URL
